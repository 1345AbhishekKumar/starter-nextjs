...
function hfRoot() {
  ...
  for (const r of roots)
    if (fs.existsSync(path.join(r, 'packages', 'cli', 'dist', 'cli.js')))
      return r;
  console.error(
    '[transcribe] hyperframes CLI not found — set HYPERFRAMES_ROOT',
  );
  process.exit(3);
}
...
function _usableWords(d) {
  return (
    ...
    d.words.some((w) => w && 'start' in w && 'end' in w)
  );
}
...
function audibleEnd(audio) {
  try {
    ...
    const starts = [...out.matchAll(/silence_start:\s*([\d.]+)/g)].map(
      (x) => +x[1],
    );
    const ends = [...out.matchAll(/silence_end:\s*([\d.]+)/g)].map(
      (x) => +x[1],
    );
...
function main() {
  ...
  if (wantWx) {
    try {
      ...
      if ((r.status || 0) !== 0)
        throw new Error(
          (r.stderr || 'whisperx failed')
            .split('\n')
            .slice(-4)
            .join(' ')
            .slice(0, 300),
        );
      const wxJson = JSON.parse(
        fs.readFileSync(path.join(outDir, '_wx_audio.json'), 'utf8'),
      );
      const wx = [];
  ...
  console.log(
    `[transcribe] text: ${text.slice(0, 160)}${text.length > 160 ? '…' : ''}`,
  );
