...
function sizePxOf(css, H) {
  ...
  if (!v) return null;
  let m;
  if ((m = v.match(/calc\(\s*([\d.]+)\s*\*\s*var\(--h\)\s*\)/i)))
...
function setFontSize(css, fracOfH) {
  ...
  if (/font-size\s*:/i.test(css || ''))
    return String(css).replace(/font-size\s*:\s*[^;}]+/i, decl);
  return (
    (css ? css.replace(/\s*$/, '').replace(/;?\s*$/, '; ') : '') + decl + ';'
  );
}
...
function main() {
  ...
  const W = plan.width || 1920,
    H = plan.height || 1080;
  const groups = (plan.groups || []).concat(
    plan.crown_group ? [plan.crown_group] : [],
  );
  ...
  for (const g of groups) {
    ...
    const nChars =
      text.replace(/\s/g, '').length + (g.words || []).length * 0.5; // glyphs + ~half-space gaps
    if (nChars < 1) continue;
    const fam = familyOf(g.css);
    let adv = ADV[fam] ?? DEFAULT_ADV;
    if (
      /text-transform\s*:\s*uppercase/i.test(g.css || '') ||
  ...
  if (changed) {
    ...
    console.log(
      `[fit-fonts] shrank ${changed} group(s) to fit; wrote plan.json`,
    );
