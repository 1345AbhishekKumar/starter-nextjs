    ...
    try {
      if (fs.existsSync(bun))
        for (const d of fs.readdirSync(bun))
          if (d.startsWith(pkg + '@'))
            cands.push(path.join(bun, d, 'node_modules', pkg));
    } catch {}
...
function ensureLayoutMeasured(project, force) {
  ...
  if (
    !stale &&
    fs.existsSync(idx) &&
    fs.statSync(idx).mtimeMs > fs.statSync(lp).mtimeMs
  )
    stale = true;
  if (stale)
    cp.execFileSync(
      'node',
      [path.join(__dirname, 'measure-layout.cjs'), project],
      {
        stdio: 'inherit',
      },
    );
...
async function main() {
  ...
  const layout = ensureLayoutMeasured(
    project,
    process.argv.includes('--remeasure'),
  );
  ...
  for (const sample of layout.samples) {
    const png = path.join(
      framesDir,
      `f_${String(sample.frame_idx).padStart(4, '0')}.png`,
    );
    const mask = await loadAlphaMask(png);
    if (!mask) continue;
    for (const cap of sample.caps) {
      const entry = (capStats[cap.id] ||= {
        layer: cap.layer || planLayer,
        samples: [],
      });
      const wordsData = [];
      for (const w of cap.words || []) {
        ...
        wordsData.push({
          text: w.text,
          occlusion: occlusionForRect(mask, w.x, w.y, w.w, w.h),
        });
  ...
  for (const [gid, entry] of Object.entries(capStats)) {
    ...
    if (entry.overflow) {
      ...
      console.log(
        `  ${gid}  [overflow${hard ? '' : '-warn'}] "${o.text}" off-frame: ${sides} (@${o.t}s)` +
          (hard
            ? ' — cropped text is always wrong'
            : ' (graze — within climax tolerance)'),
      );
      if (hard && !failures.includes(gid)) failures.push(gid);
    }
    if (entry.layer === 'fg') {
      console.log(
        `  ${gid}  fg    (occlusion skipped — fg renders above matte)`,
      );
      continue;
    }
    const capOccls = entry.samples.map((s) => s.cap_occl);
    const avgCap = capOccls.length
      ? capOccls.reduce((a, b) => a + b, 0) / capOccls.length
      : 0;
    ...
    const warn = Object.entries(wordPeaks).filter(
      ([, p]) => p >= wordWarn && p < wordFail,
    );
    let status = 'OK';
    ...
    if (heroIds.has(gid) && peakCap < 0.15) {
      ...
      const widest = Math.max(
        ...entry.samples.map((sm) => (sm.cap_bbox && sm.cap_bbox.w) || 0),
        0,
      );
