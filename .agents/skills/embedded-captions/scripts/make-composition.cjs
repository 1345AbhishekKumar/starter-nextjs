...
function findTemplate(name) {
  ...
  const avail = fs
    ...
    .sort();
  console.error(
    `[compile] unknown template: ${name}. Available: ${avail.join(', ')}`,
  );
  process.exit(1);
}
function escBr(t) {
...
function wordSpan(w, i, perLetter) {
  if (!perLetter)
    return `<span class="w" data-i="${i}">${escBr(w.text)}</span>`;
  const t = String(w.text);
  if (/<br\s*\/?>/i.test(t))
    return `<span class="w" data-i="${i}">${escBr(t)}</span>`; // explicit <br> words stay whole
  const letters = [...t]
...
function renderCap(g, heroCfg) {
  ...
  const spans = g.words
    .map((w, i) => wordSpan(w, i, perLetter))
    .join('\n            ');
...
function buildGroupsHtml(groups, planes, heroCfg) {
  if (!planes)
    return groups.map((g) => renderCap(g, heroCfg)).join('\n        ');
  ...
  return groups
    .map((g) => {
      ...
      if (isHero) {
        ...
        if (tokens.heroCss)
          dnaHero.push(
            tokens.heroCss.endsWith(';')
              ? tokens.heroCss
              : tokens.heroCss + ';',
          );
        if (tokens.hero && tokens.hero.heroColor)
          dnaHero.push(`color: ${tokens.hero.heroColor};`);
      ...
      if (isHero && tokens.hero) {
        ...
        if (tokens.hero.wordCss)
          rules.push(`      #${g.id} .w { ${tokens.hero.wordCss} }`);
...
function main() {
  ...
  if (plan.mode === 'custom') {
    console.error(
      '[compile] mode=custom — skip this script and hand-write index.html.',
    );
    process.exit(1);
  }
